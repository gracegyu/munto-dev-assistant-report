...
**PHASE 0~1 — Spec 작성 세션** *(상세: TO-BE §4.7.4)*

| 파일 | 트리거 | 자동/수동 | 커밋 |
|------|--------|----------|------|
| `spec-session-{YYYY-MM-DD}.md` | `munto-spec-writer` 스킬 호출 시 매번 | **자동 (a)** — 스킬 자체가 박음 | `main` 직접 push, 리뷰 불요 |
| `spec-review-{YYYY-MM-DD}-{문서명}.md` | `munto-spec-review` 스킬 호출 시 매번 | **자동 (a)** — 스킬 자체가 박음 | `main` 직접 push, 리뷰 불요 |
| `spec-handover-{YYYY-MM-DD}-*.md` | Spec 작성 중 사람 인계 시 | 수동 | `main` 직접 push, 인수자 Slack ack |
| **`spec-baseline-handoff.md`** | Spec baseline v1.0 동결 시점 (PHASE 1 GATE 통과 직후) | **수동 의무** — Owner 사람 작성 | `main` 직접 push, **Owner + 분석 아키텍트 ack 의무** |
...
**PHASE 2 — 구현 운영 세션** *(상세: TO-BE §4.9.7)*
