...
- TO-BE 참조: `reports/2026-05-harness-TO-BE.md` §4.3 (IP-0 ~ IP-8)
...
- [변경 관리·버전](#변경-관리버전)
- [완료 체크리스트](#완료-체크리스트)
- [Appendix A. 예시 — paid-socialing-v2.ip.md (요약)](#appendix-a-예시--paid-socialing-v2ipmd-요약)
...
| **저장 위치** | 해당 *제품 Repo* (BE/FE/APP) 의 `docs/specs/` | **`munto-dev-assistant/projects/{프로젝트명}.ip.md`** *(아래 [저장 위치와 파일명 규약](#저장-위치와-파일명-규약) 참조)* |
...
- 입력: Spec 4 종 산출물 베이스라인 v1.0 + 기존 Repo 의 `docs/specs/` + 멀티 Repo Spec 인덱스
...
| **프로젝트명 규칙** | 영문 소문자·하이픈만, 50 자 이내, 메이저 버전(v2) 만 파일명에 표기 |
| **버전 관리** | 같은 폴더에서 Git 으로 버저닝. `v1.0 → v1.1 → v1.2` 는 본문 헤더로만 표기 (파일 분기 X). **메이저 변경(v2.0)** 은 별도 파일 신설 권장 — `{프로젝트명}-v2.ip.md` |
| **인덱스** | `munto-dev-assistant/projects/README.md` 에 활성 IP 목록·상태·담당자·최신 버전 표기 |
...
- **IP** = `munto-dev-assistant/projects/` — *프로젝트 단위 메타 산출물, 멀티 Repo 를 가로지르는 단일 진입점*.
...
| 기준 | 적정값 | 너무 크면 / 너무 작으면 |
|------|--------|-----------------------|
| **PR 크기** | 변경 줄 수 200~500 줄 | 1,000 줄 초과 → 리뷰 어려움 · BLOCKER 위험. 50 줄 미만 → Task 개수 폭증 |
...
| 방식 | 설명 | 장점 | 단점 | IP 의 Spec Index 표기 |
|------|------|------|------|---------------------|
| **① 기존 Spec 수정** | BE/FE 의 `docs/specs/...` 기존 문서를 직접 수정 | *baseline 일관* — 제품과 함께 버저닝 | 큰 수정은 *diff 가 흩어져* 리뷰 어려움 | 기존 파일의 새 SHA 만 기록 |
| **② Sub스펙 누적** | `docs/specs/{기능명}/` 폴더에 신규 Sub스펙 파일 추가 (기존 문서는 그대로) | 변경 범위 명확 · 리뷰 쉬움 | 시간이 지나면 *Sub스펙 누적*으로 검색 어려움 | 신규 파일 SHA 기록 + *Sub스펙 ↔ 모스펙 연결 표기* 의무 |
| **③ 별도 repo Spec** | 별도 `project-specs/` repo 에 임시로 모은 후 구현 | *프로젝트 단위 헬리콥터 뷰* 확보 | **구현 완료 후 원본 Repo 로 통합 의무** — 통합 누락 시 영원히 임시로 남음 | 임시 SHA + ***통합 마감 Task* 가 반드시 IP 에 포함되어야 함** |
...
| **Phase 추가·삭제** | IP major 버전 (v2.0) — *사람 리뷰 재실행* 필수 (별도 파일: `{프로젝트명}-v2.ip.md`) |
...
- [ ] 저장 위치가 `munto-dev-assistant/projects/{프로젝트명}.ip.md` 인가
- [ ] 파일명이 규칙(영문 소문자·하이픈·메이저 버전만)을 따르는가
...
## Appendix A. 예시 — `paid-socialing-v2.ip.md` (요약)
...
> 전체 예시는 추후 `munto-dev-assistant/projects/_template.ip.md` 로 제공 예정.
