...
> - **PHASE 2~3 (구현·검증)** = *컨텍스트 단절* — 구현 개발자의 입력 = *IP + Spec 만*. *본 절의 PHASE 2 세션 표는 오케스트레이터·PL 모니터링용* — *구현 개발자가 읽거나 박는 파일이 아님*. 구현 개발자는 *각 제품 Repo* 에서 작업, 본인 로컬 `~/.claude/` 세션만 사용 (TO-BE §4.4 *구현 개발자 운영* 박스 참조).
>
...
| **`spec-baseline-handoff.md`** | Spec baseline v1.0 동결 시점 (PHASE 1 GATE 통과 직후) | **수동 의무** — PL 사람 작성 (*프로젝트당 1 회, `{author-id}` 불요*) | `main` 직접 push, **PL + 분석 아키텍트 ack 의무** |
...
**PHASE 2 — 구현 운영 세션** *(상세: TO-BE §4.9.7. **대상 독자 = 오케스트레이터·PL·인계자 — 구현 개발자 X**)*
...
| 변경 유형 | 절차 |
|----------|------|
| **Task Card 1 개 분리·세분화 (DoD 동일)** | IP minor 버전 (v1.1) — PL 단독 결정 가능 |
