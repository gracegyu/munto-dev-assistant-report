...
### 5.4 종합 — 행동 교정만으로 풀리지 않는다, *시스템 보강* 이 전제다

위 §5.1 ~ §5.3 의 행동 패턴은 *교육·캠페인·정신력* 으로 풀리지 않는다. 이유:
...
- §4 시스템 결함 *해결* (이미 D-1 ~ D-10 보강으로 진행)
- **+ §5 행동 패턴을 *명시적으로 막는 검증 메커니즘*** — *리뷰어 자기점검 체크리스트, AI 출력 책임 전환 원칙, 대안 검토 강제, 용어 Glossary 의무화* (TO-BE §3.2 / §3.4 / §4.7 에 추가 보강 필요).
