...
| **Spec 완료 후 (PHASE 2~3)** | *세션 컨텍스트는 단절* — 구현·테스트 단계의 *입력 = IP + Spec 만*. 작성자에게 추가 질문 없이도 누구나(AI·사람) 진행 가능해야 한다. *팀 공유 `sessions/` 의 PHASE 2 파일들(daily·phase·blocker)은 오케스트레이터·PL 모니터링용 — 구현 개발자가 컨텍스트 얻으려고 읽는 파일 아님.* | *컨텍스트가 추가로 필요하면 IP/Spec 이 불완전한 것* (IP-9.4). IP-8 *통과 = 인수* 와 일관 |

**행위자별 의무·금지 매트릭스 — 본 원칙의 *운영형*:**

| 행위자 | cwd | 입력 (읽을 것) | sessions/ 폴더 관계 |
|--------|-----|---------------|--------------------|
| **Spec 작성자 (분석 아키텍트)** | `munto-dev-assistant/projects/{프로젝트명}/` | SRS · DBML · Swagger · UI/TCL · 본인의 `spec-session-*.md` 누적 | **작성 의무** — `spec-session-*.md`·`spec-review-*.md`·`spec-baseline-handoff.md` (PL) |
...
| **PL / 리뷰어** | 상황별 | 위 산출물 전부 | 읽기 권한 — *직접 박는 의무 없음* (오케스트레이터/Spec 작성자가 박은 것을 읽음) |
...
3. **인계가 발생하면 — 작성자→작성자, 작성자→ip-writer, 작성자→PL — 인계 단일 컨텍스트는 IP (PHASE 2 진입 시) 또는 `spec-baseline-handoff.md` (PHASE 1 GATE 통과 시).** 그 외 정보는 *읽으면 도움이 되지만 안 읽어도 진행 가능* 해야 한다.
...
5. **PHASE 2 sessions/ 의 daily/phase/blocker 는 *모니터링 데이터*** (오케스트레이터·PL 이 *프로젝트가 잘 굴러가는가* 추적용) — *구현 개발자가 컨텍스트 얻으려고 읽는 파일 아님*. *상세는 §4.9.7*.
...
| 섹션 | 내용 |
| --- | --- |
| **1. Project Header (프로젝트 헤더)** | 프로젝트명 / PL (Slack 핸들) / 작성 시작·현재 버전 (v1.x) / 관여 Repo 목록 + 각 Repo 의 *베이스라인 SHA* / Operating Mode 디폴트 / Slack 채널 / Kill Switch 위치 |
...
| **구현 개발자가 `sessions/` 에 본인의 일일/Task 진행 로그를 박음** *(§2.3 ⑧ 컨텍스트 단절 원칙 위반)* | `sessions/` 폴더가 *오케스트레이터·PL 시각의 모니터링 데이터* 와 *구현 개발자 개인의 진행 메모* 가 섞여 오염 — 인계 시 *어떤 게 공식 기록인지 모호*, IP/Spec 단일 진실 원칙 깨짐 | 구현 개발자의 진행은 *본인 로컬 `~/.claude/` 세션 + PR description* 으로만. 팀 공유 `sessions/` 는 *오케스트레이터·Spec 작성자·인계자* 만 작성 — §4.4 *구현 개발자 운영* 박스 참조 |
...
> | **본인의 진행 로그** | *본인 로컬 `~/.claude/` 세션 자동 보존 + PR description 만*. **팀 공유 `sessions/` 폴더에 본인의 일일/Task 작업 로그 박지 않음** | sessions/ 가 오케스트레이터·PL 시각과 다른 noise 로 오염 — 인계 시 어떤 게 *공식 기록* 인지 모호. IP-9.6 안티 패턴 위반 |
>
...
| PHASE 2 | PR 승인 | **PR *승인*(머지) = 인수.** 1 차 리뷰는 AI, 사람의 *직접 정독* 은 *risk-tier*(인증·인가·결제·정산·DB 마이그레이션·보안/개인정보)에 집중 (§4.7.6) | diff 전수 육안 ≠ 게이트. 진짜 위험은 risk-tier 에 몰려 있음 — 사람 집중력을 거기 배분 |
| PHASE 3 | 수동 | 수동 테스트 항목 수행 + 실패 시 Jira 이슈 등록. **수동 항목은 IP 의 수동 DoD·risk-tier 에서 도출** (§4.7.6 ④) | 자동화 불가 영역 (UI 자동화 숙제 §3.6 참조). *무엇을 사람이 볼지* 는 IP 에 사전 명시 |
...
| **`spec-baseline-handoff.md`** | Spec baseline v1.0 동결 시점 (PHASE 1 GATE 통과 직후) | PL (사람) + ip-writer 참조 | **수동 의무** — *프로젝트당 1 회, PL 단독 작성이므로 `{author-id}` 불요*. *ip-writer 가 IP 초안 생성 시 반드시 이 파일 우선 참조* |
...
| `spec-baseline-handoff.md` | **(가장 중요)** baseline v1.0 동결 일자·문서 경로 4 종 (SRS·DBML·Swagger·UI/TCL)·각 문서의 동결 SHA·*핵심 아키텍처 결정 3~5 개* (§4.7.3 대안 검토 박스에서 *채택안·기각 대안·재검토 조건* 만 추출)·*미해결 TBD 잔여 목록* (해소 책임자 + 결정 시점)·*PHASE 2 진입 시 ip-writer 가 우선 참조해야 할 5 가지 컨텍스트*·PL 사인·분석 아키텍트 사인 |
...
§4.9.7 (3) *PHASE 2 sessions/ append-only 정책* 과 동일 — *세션 파일은 운영 기록* 이므로 `sessions/` 폴더 안에서는 *PR 없이 main 직접 push 허용*, 리뷰 불요. 단 **`spec-baseline-handoff.md` 는 예외** — PHASE 1 GATE 와 연동되므로 *PL + 분석 아키텍트 1 인 ack* 후 push.
...
| `spec-baseline-handoff.md` | PHASE 1 GATE 통과 시점 | `main` 직접 push | **PL + 분석 아키텍트 ack 의무** |
...
| **인계 (`spec-handover-*.md`)** | 사람 (PL / 인계자) | 자동화 대상 아님 |
| **baseline 동결 (`spec-baseline-handoff.md`)** | PL (사람 작성). `munto-spec-review` 스킬이 *마지막 리뷰가 PASS 일 때* *이 파일이 존재하는가* 자동 점검 → 없으면 BLOCKER | **본 PR 에 포함** — `munto-spec-review` SKILL.md 갱신 |
...
|---|--------------|----------|--------------|--------|
| 1 | **`munto-dev-assistant/repo-inventory.md`** | Munto 가 다루는 *모든 Repo* 의 이름·역할·기본 브랜치·docs/ 존재 여부·Repo Owner 1 인. 신규 Repo 추가 시 본 인벤토리 PR 갱신 의무 | 분석 아키텍트 + 백엔드 리드 | Repo 신설/폐기·도메인 재배치 시 |
...
| 3 | **`munto-dev-assistant/multi-repo-spec-index.md`** | *멀티 Repo Spec 인덱스 실체* — `ip-standard.md` 가 언급한 *인덱스* 의 *진짜 산출물*. 컬럼 = `Spec ID (S-BE-1 등) / Repo / 경로 / 도메인 / 마지막 baseline SHA / 마지막 갱신일 / Repo Owner`. *PHASE 1 Spec 베이스라인 동결 시 자동 1 행 append* | ip-writer + Repo Owner | 새 Spec 베이스라인 동결 시 |
...
| 5 | **`dev-chain-design/SKILL.md` *입력 확인* 절 갱신** | 현 본문의 *기존 Swagger URL* 1 줄을 *기존 Repo docs/ 탐색 4 단계* 로 확장 — (i) 인벤토리 조회 (ii) 도메인 매칭 (iii) `docs/specs/` 트리 탐색 (iv) 인덱스 등록 + Repo Owner 확인 | dev-chain-design 운영자 | 1~3 완료 후 |
...
#### 4.7.6 코드 리뷰 — AI 시대 PR 리뷰·승인 원칙 (risk-tier)

*(AS-IS §5.1 · §5.5 행동 패턴 대응)* AI 가 대량 생산한 코드의 *diff 를 사람이 전수 육안 리뷰* 하는 것은 *게이트로서 가치가 낮다* — 결함 발견율·학습 효과 모두 미미하고, 오히려 *"리뷰했으니 통과"* 라는 형식적 안심만 준다. **리뷰의 무게중심은 코드(PHASE 2)에서 *스펙·설계(PHASE 0·1)* 로 이미 옮겨졌다** (§3.2 · §3.4 의 사람 게이트가 진짜 게이트다). 본 절은 *AI 시대의 PR 리뷰·승인* 을 4 가지 기둥으로 정식화한다.

| # | 기둥 | 운영 |
| --- | --- | --- |
| ① | **Diff 전수 육안 리뷰 ≠ 게이트** | AI 가 만든 변경을 *사람이 한 줄씩 읽는 것* 은 게이트가 아니다. *진짜 게이트* = PHASE 0·1 스펙/설계 리뷰 + PHASE 1 종합 Peer Review (베이스라인). 코드 단계의 정밀도는 *스펙 정확도* 에서 나온다 (§2.3 ①·⑧) |
| ② | **PR 1차 리뷰 = AI** | PR 의 1 차 리뷰는 *AI 가 수행* 한다 (도메인 규칙 위반·정합성·테스트 누락·명백한 결함). 사람이 diff 를 처음부터 훑는 일을 대체한다. AI 리뷰 결과(점검 항목·근거)는 PR 본문에 *영수증* 으로 남긴다 |
| ③ | **사람은 PR 을 *승인* 한다 — 통과 = 인수** | 머지 권한은 사람. *CI 그린 = 자동 머지* 금지 (§4.9.5). 머지자는 *AI 가 짰다* 는 이유로 면책되지 않는다. **단, 사람의 *직접 정독* 은 *risk-tier 상위* 에 집중** — 아래 표 |
| ④ | **동작 검증 = AI 자동화 우선, 수동분은 IP 에 사전 명시** | 최종 동작은 Unit·E2E 로 *최대한 자동화* (§2.3 ⑥, §3.6). *AI 가 자동화할 수 없는 확인* 만 사람이 한다. **그 *사람 수동 확인 항목* 은 IP 의 *수동 DoD* (`mode: manual`) + Task `risk` 필드에 *사전* 명시** 되어야 한다 — 구현 완료 후에야 *"이건 사람이 봐야 하나?"* 를 묻는 일이 없도록 (§4.3 IP DoD) |

> **risk-tier — 사람이 *직접 정독* 해야 하는 영역**
>
> | 영역 | 왜 사람이 직접 보나 | 자동화로 충분한가 |
> | --- | --- | --- |
> | **인증·인가** (auth) | 권한 경계 오류는 자동 테스트가 *놓친 케이스* 에서 사고가 남 | ❌ — 사람 직접 |
> | **결제·정산** (payment) | 금액·환불·정산 로직은 *돈* 이 직접 걸림 | ❌ — 사람 직접 |
> | **DB 마이그레이션** (migration) | 운영 데이터 손실·되돌리기 불가 | ❌ — 사람 직접 |
> | **보안·개인정보** (security/PII) | 유출 시 회복 불가, 법적 책임 | ❌ — 사람 직접 |
> | 그 외 일반 CRUD·UI·리팩토링 | 도메인 규칙 위반은 AI 1 차 리뷰 + Unit/E2E 로 커버 | ✅ — AI 리뷰 + 자동 테스트 |
>
> *risk-tier 외 영역까지 사람이 전수 정독하면 진짜 위험 지점에 쓸 집중력이 분산된다.* 사람 정독 시간은 *희소 자원* 이므로 risk-tier 에 배분한다.

> **에이전트 강제 방법**
> - `munto-create-pr` — PR 생성 전 *AI 1 차 셀프 리뷰* (도메인 규칙·정합성·테스트 누락) 수행 + PR 본문에 *Review guide*(risk-tier 터치포인트 + IP 의 *사람 수동 확인 항목*) 자동 작성. PR 생성 완료 후 **`munto-pr-review` 를 자동 호출**. 머지는 사람.
> - **`munto-pr-review`** *(신설)* — *생성된 GitHub PR 에 실제 리뷰를 게시* 하고 반자동 코멘트→수정 루프를 운영 (§4.7.6.1).
> - `dev-chain-verify` — Unit·E2E 자동화 우선. *수동 체크리스트* 는 *IP 의 수동 DoD·risk-tier* 에서 도출 (AI 가 임의 생성하지 않음).
> - `dev-chain-implementation-plan` / `ip-writer` — *사람 수동 확인 항목*(특히 risk-tier)을 IP 의 *수동 DoD*(`mode: manual`) + Task `risk` 필드에 사전 명시. `ip-reviewer` 는 *risk-tier 로 보이는 Task 에 수동 DoD 가 없으면* 🟡 WARNING.
>
> **리뷰 시 사용**: 무인 모드 PR (§4.9) 도 동일 — *AI 1 차 리뷰 → 사람 승인(risk-tier 정독) → 머지=인수*. 자기점검 5 개 질문(§3.2 (2)) 동일 적용.
>
> *(참조: §2.3 ① 정확함 ≠ 자세함, §3.2 · §3.4 통과 = 인수, §3.6 테스트 자동화, §4.3 IP DoD, §4.9.5 무인 PR)*

##### 4.7.6.1 운영 — `munto-pr-review` 스킬과 반자동 리뷰 루프

기둥 ②(PR 1 차 리뷰 = AI)의 *실행 도구* 다. **수단 결정**: *외부 자동 리뷰 서비스(Bugbot·CodeRabbit 등)를 쓰지 않고, 자체 단일 스킬 `munto-pr-review` 하나로 운영* 한다 — 이유: 우리 게이트의 핵심(Swagger/DBML 계약 정합성·도메인 규칙 위반·risk-tier·IP DoD 충족)은 *diff 만으로는 검출 불가* 하고 *Spec/IP 컨텍스트* 가 필요한데, 외부 서비스는 그 컨텍스트를 모르기 때문이다. *코드 diff 만 보는 리뷰는 일반 버그 그물로는 쓸만하지만 우리 게이트로는 부족하다.*

> **입력 (diff 만이 아니다)**: ① PR diff(`get_pull_request_files`) ② 해당 Task 의 IP DoD·`risk`(`projects/{프로젝트}/ImplementationPlan.md`) ③ Swagger/DBML 베이스라인 ④ 도메인 규칙(`.agents/rules/{backend|frontend|mobile}`). 이 4 입력으로 *Spec 정합성·도메인 규칙·risk-tier* 를 점검해 GitHub PR 에 리뷰를 게시한다(`create_pull_request_review`).
>
> **AI 는 *APPROVE 하지 않는다*** — `event` 는 결함 있으면 `REQUEST_CHANGES`, 없으면 `COMMENT`. **APPROVE(승인=머지 자격)는 사람의 권한** (기둥 ③).

**반자동 코멘트→수정 루프** *(사용자 결정: 전면 자동 아님)*

```mermaid
flowchart TD
    PR["munto-create-pr: PR 생성"]:::ai --> REV["🤖 munto-pr-review<br/>(Spec/IP·도메인·risk-tier 리뷰 게시)"]:::ai
    REV --> CLS{미해결 코멘트?}:::gate
    CLS -- "없음 + CI green" --> CONV["✅ 수렴 → audit trail 생성"]:::ai
    CLS -- "일반(스타일·버그·정합성·테스트)" --> AUTO["🤖 자동 수정<br/>(도메인 구현 스킬 위임) → push"]:::ai
    CLS -- "risk-tier · 설계 변경" --> HUMAN["👤 사람 확인 후 수정 방향 결정"]:::human
    AUTO --> REV
    HUMAN --> REV
    CONV --> APPROVE{"👤 사람 승인<br/>audit trail 요약 + risk-tier 직접 정독"}:::gate
    classDef ai fill:#e0f2fe,stroke:#0284c7,color:#0c4a6e
    classDef human fill:#fef3c7,stroke:#d97706,color:#78350f
    classDef gate fill:#fee2e2,stroke:#dc2626,color:#7f1d1d,stroke-width:3px
```
...
| 코멘트 유형 | 처리 |
| --- | --- |
| 스타일·명백한 버그·테스트 누락·Swagger/DBML 정합성 | **자동 수정 루프** — 도메인 구현 스킬에 위임 → push → `munto-pr-review` 재호출(재리뷰). 같은 코멘트 *재시도 N 회 상한* (초과 시 BLOCKER → 사람) |
| **risk-tier**(인증·인가·결제·정산·DB 마이그레이션·보안/개인정보) | **사람 확인 후** 수정 방향 결정 (자동 수정 금지) |
| 설계·아키텍처 변경 요구 | **사람 판단** — 베이스라인 변경이면 §4.8 변경 관리(CCB)로 |

> **"미해결 코멘트" 는 두 종류 — 전부 사람이 처리하는 게 아니다**: *일반* 코멘트는 *AI 루프가 자동 해소*(수정→push→재리뷰, 사람 개입 X), *risk-tier·설계* 코멘트만 *사람이 확인·방향 결정* 해야 해소된다. (대화 resolve 도구는 GitHub MCP 에 없어 `gh api graphql` 또는 사람 승인 시 정리)

**사람 핸드오프 — *어떻게 알리나*** (risk-tier·설계로 정지할 때):
> - **유인(대화 세션)**: 루프를 돌리는 *에이전트가 채팅으로 즉시 보고* 후 대기.
> - **무인(야간 파이프라인)**: *PR 에 흔적 + Slack* — `gh pr edit --add-label needs-human-review`(+risk 라벨) + 담당자 지정(`--add-assignee`/`--add-reviewer`) + @멘션 코멘트 + 프로젝트 *Operating Mode(§4.9 / 헤더 §7)* 의 **Slack 알림 정책**대로 통지.

**재리뷰 트리거 — *누가 다시 실행하나***: ① *루프 안 AI 수정→push* 또는 *유인에서 사람이 방향만 지시→AI 수정* → **자동**(같은 세션 AI 가 push 직후 재실행). ② *세션 밖에서 사람이 직접 코드 수정→push* → **현재 수동**(사람이 "다시 리뷰" 요청; 향후 CI `push` 트리거로 자동화 가능). 이로써 *PR 생성 → 리뷰 → 수정 → 재리뷰* 가 한 사이클로 돈다.

**사람 최종 승인 — *diff 전수가 아니라 audit trail*** *(기둥 ③ 의 실행형)*

루프가 *수렴*(미해결 코멘트 0 + CI green)하면 정지하고 **리뷰 라운드 audit trail** 을 PR 에 누적 기록한다. 사람은 *전체 diff* 가 아니라 이 표 + *risk-tier 표시 항목만 직접 정독* 후 승인(머지)한다.

| 라운드 | 코멘트(주체) | risk-tier | AI 수정 내용 | 재검증 |
| --- | --- | --- | --- | --- |
| R1 | "환불 금액 음수 가능" | payment 🔴 | guard 추가 + UT-201 | ✅ test green |
| R2 | "DTO snake_case" | — | camelCase 변경 | ✅ |

> *지적 내용과 AI 가 어떻게 고쳤는지를 모두 검토* 하되, *읽는 형태* 가 diff 정독이 아니라 *구조화된 지적-수정 매핑 + risk-tier 집중* 이다. risk-tier 코멘트가 한 번이라도 있었으면 그 수정 부분은 사람 직접 정독 필수.
>
> *(무인 모드(§4.9)에서도 동일 루프. 단 risk-tier·설계 변경 코멘트는 BLOCKER 정지 → 사람 위임. 검증 주체 분리(§4.9.3)·Kill Switch(§4.9.6) 그대로 적용.)*

---

### 4.7.7 외부 스킬 패턴 흡수 (mattpocock/skills — Munto 변형)

> **배경**: 외부 오픈소스 [mattpocock/skills](https://github.com/mattpocock/skills)("Skills For Real Engineers")의 검증된 패턴 중 *Munto 게이트를 강화하는 것만 선별*해 흡수한다. **원형을 통째로 이식하지 않는다** — Munto 기존 산출물(SRS §1.4 용어집·Decision Log·TCL·IP·`code-investigator`)과 연결되도록 재작성한다. 흡수 4종은 모두 *기존 PHASE 의 보강*이며 새 PHASE 를 만들지 않는다.

| 외부 패턴 | 흡수 형태 | Munto 적용 위치 | 연결되는 기존 자산 |
| --- | --- | --- | --- |
| `grill-with-docs` / `grill-me` (착수 전 1문1답 정렬 인터뷰) | **합침** | `munto-spec-writer` Step 0 「착수 전 정렬 인터뷰(그릴링)」 (선택) | 용어 충돌 지적 → §1.4 / 모호어 정밀화 / 트레이드오프 결정 → Decision Log |
| `grill-with-docs` ADR 절제(3조건) | **합침** | `munto-spec-change` Decision Log 휴리스틱 + `munto-spec-writer` 대안 검토 박스 | 되돌리기 어려움+맥락없이 의외+진짜 트레이드오프 셋 다 참일 때만 기록 |
| `diagnose` (피드백 루프 우선 디버깅) | **신규 스킬** | `munto-diagnose` (6 Phase: 피드백 루프→재현→가설→계측→수정+회귀→정리) | 분석=`code-investigator` / 수정=`dev-chain-*` / 회귀=`dev-chain-verify` / 이슈화=`munto-create-issue` |
| `tdd` (수직 슬라이스·행위 테스트) | **합침** | `dev-chain-verify` Step 1 판정 기준 + 도메인 스킬 Unit Test 절 | TCL ↔ 실제 테스트 1:1 대조의 *품질 기준* |

> **미도입 결정**: `zoom-out`(`code-investigator` 로 충분), `to-prd`·`to-issues`·`triage`(Munto SRS·IP·Jira 가 더 엄격), `caveman`·`handoff`·`write-a-skill`(개인 생산성/중복). *우리 프로세스에 이미 더 강한 게이트가 있는 영역은 차용하지 않는다.*

> **그릴링의 의미**: 가장 흔한 실패는 *AI 가 의도를 잘못 이해한 채 그럴듯하게 써버리는 것*이다. 착수 전 그릴링은 이 실패를 *Spec 작성 시작 시점*에 차단한다 — PHASE 0 의 *사람이 핵심 문단 먼저 작성*(§3.4) 원칙과 같은 방향(AI 통째 위임 금지)이다. **개발자가 도입한 외부 스킬을 팀 표준으로 흡수한 첫 사례** — 향후 외부 스킬 도입은 *기존 자산과의 연결 + 미도입 사유 기록*을 동일하게 따른다.
...
> **운영 스킬**: 본 절의 절차·템플릿은 `munto-spec-change` 스킬로 운영한다(트리거: "스펙 변경", "변경 영향 평가"). **CCB 의사결정은 아직 강제하지 않되, AI 1차 영향 평가(§4.8.4)는 변경 시 의무.** 영향 평가는 *신속함 > 완벽함*(대부분 몇 시간 내)으로 수행하고, 요청자 설득의 근거 자료로도 쓴다. 변경 요청서·영향 체크리스트(비즈니스·기술)·보고서 템플릿은 스킬 본문 참조.
...
| **PR 1 차 리뷰** | ✅ 루프 안 | — | `munto-pr-review` 가 GitHub PR 에 리뷰 게시 (Spec/IP 정합성·도메인 규칙·risk-tier) + PR 본문 *Review guide* 영수증 (§4.7.6 ②·§4.7.6.1) |
| **코멘트→수정 루프** | ✅ 일반 / ❌ risk-tier·설계 | (risk-tier·설계) ✅ | 일반 코멘트는 자동 수정→push→재리뷰. risk-tier·설계 변경 코멘트는 BLOCKER 정지 → 사람 위임 (§4.7.6.1) |
| **PR 머지(승인)** | ❌ | ✅ | 사람이 직접 머지 — *통과 = 인수* (§3.2 · §4.7). audit trail 요약 + *risk-tier* 직접 정독 (§4.7.6 ③·§4.7.6.1) |
...
| **AI 1 차 리뷰 선행** | 머지 전 AI 1 차 셀프 리뷰가 PR 본문에 *Review guide* 영수증으로 남아야 함 (§4.7.6 ②) — 도메인 규칙·정합성·테스트 누락·risk-tier 터치포인트 |
| **머지 = 인수** | 머지자는 *AI 가 짰다* 는 이유로 면책되지 않는다. §3.2 (1) *통과 = 인수* 원칙이 무인 PR 에도 그대로 적용 |
| **사람 직접 정독 = risk-tier** | diff 전수 육안 ≠ 게이트. 사람은 *인증·인가·결제·정산·DB 마이그레이션·보안/개인정보* 영역만 직접 정독 (§4.7.6 ③) — 나머지는 AI 1 차 리뷰 + 자동 테스트에 위임 |
| **PR 본문 의무 표기** | 자동 생성 라벨, 트리거 사람, 베이스라인 버전, 자체 검증 · 정합성 검증 결과, 변경 영향 범위, 머지 책임자 후보, *AI Review guide · IP 의 사람 수동 확인 항목* |
...
> **본 절은 §2.3 ⑧ *Spec ↔ 구현의 컨텍스트 단절 원칙* 의 *PHASE 2 운영형*.** 다음 절대 오해 금지 — *본 절의 sessions/ 파일들은 **오케스트레이터·PL·인계자의 모니터링·추적용** 이지 **구현 개발자가 컨텍스트를 얻기 위해 읽는 파일이 아니다***. 구현 개발자의 *유일한 입력* 은 IP + Spec (§2.3 ⑧ 행위자 매트릭스). 구현 개발자가 본 sessions/ 를 읽어야 일이 되면 *IP 가 불완전* 한 것이며 IP 작성 단계로 돌아가야 한다 (IP-9.4).
>
...
> - ✅ **PL / 분석 아키텍트** (프로젝트 진척·BLOCKER·비용 모니터링)
...
|----|----|----|----|----|
| **`YYYY-MM-DD-daily-summary.md`** | 수동 (선택) | **자동 의무** ✅ | 매일 정해진 시각 (디폴트 07:00 KST — 출근 전, 야간 작업 종료 후) | 오케스트레이터 (무인) / PL (유인) |
| **`YYYY-MM-DD-phase-{n}-summary.md`** | 수동 (선택) | **자동 의무** ✅ | Phase 완료 이벤트 발생 시 (모든 Task DoD 통과 직후) | 오케스트레이터 (무인) / PL (유인) |
...
| 트리거 | 자동 생성 책임자 (권장 구현) |
|--------|--------------------------|
| 매일 07:00 KST (`daily-summary.md`) | GitHub Actions Cron Workflow (예: `.github/workflows/daily-session-summary.yml`) — 무인 오케스트레이터 사이클 종료 후, 출근 전 |
...
| 인계 (`handover-*.md`) | 사람 (PL / 인계자) — 자동화 대상 아님 |
...
| 일자 | 내용 |
| --- | --- |
| 2026-06-05 | **일일 요약 Slack 알림 시각을 07:00 KST 로 통일** *(사용자 결정: 09:00·00:00 혼재 + 09:00 은 출근 시점과 겹쳐 "출근했을 때 요약이 와 있지 않음" 문제)* — §4.9.6 알림 묶음 정책·§4.9.7 (1) 파일 매트릭스·§4.9.8 자동 생성 책임자 표의 일일 요약 시각을 *07:00 KST(출근 전)* 로 변경. 근거 문구도 *"야간 작업 종료 직후"* → *"출근 전, 야간 작업 종료 후"* 로 보강. `ip-standard.md`(3 곳: 세션 매트릭스 트리거·Operating Mode·사람 알림 시점) / `_template/ImplementationPlan.md` §7 / 임시 미러본(`ip-standard.md`·`ImplementationPlan.md`) 동기화. **핵심 메시지: 일일 요약은 출근해서 바로 확인할 수 있도록 07:00 KST 발송으로 전 문서 통일.** |
