...
| **무인 실행 루프(§4.9) 가 변경 필요 발견** | ✅ 본 절차 필수 | 루프가 BLOCKER 로 정지 → §4.8.4 영향도 분석 템플릿을 채운 Jira 이슈 자동 등록 → 본 절차 진입. 무인 루프는 베이스라인 산출물을 직접 수정하지 않는다 |
...
>
> 무인 루프는 **PHASE 2 안에서만 작동**한다. *PHASE 0/1 의 사람 게이트(A4b · A6 · D3 · S3 · U2 · T5 · GATE)와 PR 머지는 무인화 대상이 아니다.* 이 경계가 무너지면 §3.2 · §3.4 의 *통과 = 인수* 원칙이 자동 통과로 무력화된다.
...
| 구분 | 조건 |
| --- | --- |
| **트리거** | ① 사람이 PHASE 1 게이트 통과 후 무인 모드 켜기 / ② 새 베이스라인 v1.x 가 확정되면 자동 시작 / ③ Jira 이슈 라벨(예: `auto-impl`) 부착 시 |
| **정상 종료** | TCL 커버리지 · 검증 · PR 생성 완료 → Slack 알림 → 사람 머지 대기 |
| **자동 정지 (Auto-stop)** | ① 동일 실패 *N 회 연속* / ② 비용 상한 도달 / ③ 베이스라인 산출물 수정 필요 발견 / ④ 외부 시스템 응답 불가 / ⑤ 야간 작업 윈도우 종료 시각 |
...
| **알림** | 시작 · 단계별 결과 · BLOCKER · 종료 모두 Slack 채널 통보 + Jira 코멘트 |
