...
>
> - [ ] **이해 점검**: 이 SRS 의 *모든 용어·약어*를 내가 설명할 수 있는가? 못하면 §4.7.2 Glossary 부록에서 정의 확인. 정의가 없으면 작성자에게 질문 → Glossary 보강 → 다시 리뷰
...
>
> 1. **Glossary 조회**: §4.7.2 Glossary 부록에서 정의 확인
> 2. **정의 없음 → 작성자에게 질문**: Slack/회의에서 명시적으로 *"○○ 가 무엇입니까?"* 질문 (모른다는 사실을 *숨기지 않는다*)
> 3. **Glossary 보강 요구**: 답을 들었으면 Glossary 부록에 정의 추가 요청
> 4. **재리뷰**: Glossary 보강 후 다시 정독
>
...
>
> ③ **모르는 용어 발견 시 질문 강제** — 특히 PHASE 1 은 *기술 용어가 폭증*하는 구간. *"NOT NULL DEFERRABLE", "JWT refresh rotation", "soft delete", "optimistic locking"* 등 용어를 *들어본 적은 있지만 정확히 설명 못하는* 상태로 통과시키는 것 금지. §4.7.2 Glossary 보강 절차 따른다.
>
...
| 섹션 | 내용 |
| --- | --- |
| **1. 프로젝트 헤더** | 프로젝트명 / 베이스라인 버전 (v1.x) / IP 버전 / 작성자(분석 아키텍트·리더 실명) / 관여 Repo 목록 + 각 Repo 의 *기준 commit SHA* |
| **2. Spec 인덱스** | 4 종 산출물의 위치 — 각각 *(Repo · 경로 · 섹션 Anchor · 베이스라인 SHA)* 4 요소로 표기 (IP-3 참조) |
| **3. Phase 분해** | 굵직한 작업 단위 (예: BE 데이터 모델 / BE API / FE 페이지 / App 화면 / 통합 검증). Phase 간 순서·병렬성 명시 |
| **4. Task 분해** | Phase 안 Task 목록. 각 Task = AI 1 회 실행 단위 (IP-2 기준) |
| **5. Task 별 카드** | 각 Task 별 다음 필수 필드 (아래 표) |
| **6. 의존성 DAG** | Task 간 전제·후속 관계. Mermaid `flowchart LR` 권장 |
| **7. DoD 매핑** | Task 별 완료 판정 기준 = TCL 케이스 ID + lint/typecheck/test 통과 (IP-5) |
| **8. 변경 이력** | IP 자체의 v1.0 → v1.x 변경 기록. Spec 베이스라인 변경 시 IP 도 동기화 |
...
*Spec 작성 ~ IP 작성 ~ 무인 실행 트리거* 를 같은 세션에서 진행할지(*세션 통합*), Spec 과 구현을 다른 세션으로 나눌지(*세션 분리*) 는 프로젝트 리더가 매번 판단한다. **TO-BE 는 어느 쪽도 디폴트로 강요하지 않는다.** 다만 다음 4 기준을 명시적으로 본 뒤 결정한다.

| 기준 | 세션 통합이 유리 | 세션 분리가 유리 |
| --- | --- | --- |
| **관여 Repo 수** | 1~2 개 | 3 개 이상 |
| **예상 Task 수** | 적음 (예: ≤ 20) | 많음 |
| **기간** | 짧음 (예: 1~2 스프린트) | 김 |
| **인원** | 리더 1 인 주도 | 다수 분담 |
...
| **자기점검 항목** | (1) Task 단위가 IP-2 기준에 부합? (2) 모든 Task 가 Spec 참조 경로 4 요소 갖춤? (3) 의존성 DAG 가 끊김·순환 없음? (4) 모든 DoD 가 기계 판정 가능? (5) BLOCKER 위험 항목에 대응 계획? |
...
#### 4.7.2 Glossary(용어집) 의무화 — *"모르는 용어 그냥 넘기기"* 행동 패턴 차단

*(AS-IS §5.2 행동 패턴 대응)* AI 시대에 가장 위험한 행동 중 하나가 *모르는 용어를 그냥 넘기는 것*이다. SRS·DBML·Swagger 의 핵심 용어가 *팀 안에서 다르게 이해되거나, 누구도 정확히 모른 채 통과*되면 6 개월 뒤 *"왜 이렇게 만들었는지 아무도 모르는 시스템"* 이 된다. 이를 막기 위해 모든 Spec 산출물에 **Glossary 부록 의무화**.

| 항목 | 운영 |
| --- | --- |
| **위치** | SRS 부록 (예: SRS §10 Glossary), DBML 파일 상단 주석, Swagger `components/schemas` 의 description |
| **포함 대상** | ① 도메인 용어 (예: *"소셜링", "호스트", "Munto Pay"*) ② 기술 용어 중 *팀 안에서 정의가 갈릴 수 있는 것* (예: *"soft delete", "idempotency key"*) ③ 약어·외래어 (예: *"PG", "CCB", "TBD"*) ④ AI 가 사용했지만 *왜 그 용어를 골랐는지 즉시 알기 어려운 것* |
| **각 항목 필수 정보** | 용어 / 한 줄 정의 / 우리 프로젝트에서의 *구체적* 의미 (일반 정의가 아닌) / 헷갈리기 쉬운 유사 용어와의 차이 |
| **에이전트 강제** | `munto-spec-writer` 가 SRS 작성 시 *본문에 등장한 모든 비표준 용어* 를 자동으로 Glossary 부록에 등록 (정의는 사람이 채우거나 AI 가 1 차 작성). `munto-spec-review` 는 *Glossary 누락 용어* 를 결함으로 잡는다 |
| **리뷰 시 사용** | §3.2 사람 리뷰 자기점검 체크리스트의 *이해 점검* 항목과 연결. 리뷰어가 모르는 용어 → Glossary 조회 → 정의 없으면 작성자에게 질문 → Glossary 보강 → 재리뷰 |

> **운영 팁**
> - Glossary 가 *비어 있으면 안 된다*. *"용어 정의가 필요한 게 없습니다"* 라는 답은 *작성자가 본인이 모르는 용어가 무엇인지 모른다* 는 신호일 가능성이 높다 — AI 보조로 본문에서 *최소 5 개* 는 뽑게 한다.
> - 신규 입사자에게 SRS 를 정독시킨 뒤 *"모르는 용어 30 분 안에 뽑아보라"* — 거기 안 적힌 게 진짜 Glossary 후보다.
>
> *(참조: 국내 SW 스펙 작성 표준 §6.11 「용어 정의」)*
...
> **에이전트 강제 방법**
> - `munto-spec-writer` 가 *핵심 아키텍처 결정* 으로 분류되는 항목을 발견하면, 위 박스를 *자동 생성* 하고 AI 1 차 대안 2 개 작성 → 사람이 검토·보강·결정.
> - `dbml-writer` / `swagger-writer` 가 *주요 엔티티·계약 패턴* 에 대해 동일 박스 자동 첨부.
> - `munto-spec-review` / `dbml-reviewer` / `design-consistency-reviewer` 는 *핵심 결정에 대안 검토 박스가 없으면 결함* 으로 잡는다.
>
> **리뷰 시 사용**: §3.2 / §3.4 사람 리뷰 자기점검 체크리스트의 *대안 점검* 항목과 연결. 리뷰어가 *"대안 검토 박스 없음 = 결함"* 으로 통과 보류.
>
