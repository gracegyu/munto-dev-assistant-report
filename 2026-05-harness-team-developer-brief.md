...
| **Implementation Plan (구현계획서, IP)** | PHASE 1 의 *마지막 활동*. Spec 을 *무인 실행 가능한 형태로 변환한 단일 문서*. Task 별 *Spec 참조 경로 4 요소 · DoD · 의존성 DAG · Repo 매핑* 포함. PHASE 2 (유인/무인 §4.9) 의 *유일한 입력*. **저장 위치: `munto-dev-assistant/projects/{프로젝트명}.ip.md`** *(Spec 은 BE/FE 각자 Repo 에 baseline 으로, IP 는 멀티 Repo 를 가로지르는 단일 진입점 — TO-BE §4.3 IP-0 참조)* | "그냥 Task 리스트" → "**IP v1.x — `munto-dev-assistant/projects/` 아래 단일 파일**" |
