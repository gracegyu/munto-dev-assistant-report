...
| ⑧ | **Spec ↔ 구현의 컨텍스트 단절 원칙** *(NEW)* | PHASE 0~1 (Spec 작성) = *컨텍스트 적극 보존* (세션·핸드오프·결정 이력 박음). PHASE 2 (구현) = *컨텍스트 단절* (IP / Spec 만으로 누구나 — AI·사람 모두 — 구현·테스트 가능). 구현 개발자가 Spec 작성 세션을 봐야 한다면 *Spec 이 부족하다* 는 신호. ⑦ 무인 실행의 *전제* |
