...
### TO-BE 가 AS-IS 대비 추가/강화한 것 (요점 8 가지)
...
| 8 | **PHASE 2 무인 실행 모드 (오케스트레이션 루프)** *(NEW)* | 야간 무인 구현·검증·PR 생성을 위한 운영 규칙. **무인화 범위는 PHASE 2 구현·검증·PR 생성까지** — PHASE 0/1 사람 게이트와 PR 머지는 무인화 비대상. 검증은 *오케스트레이터가 판정* (에이전트 자기 보고 X), 베이스라인 산출물 불변 |

### 우리 팀이 *오늘부터* 다르게 해야 할 일 — 5 줄
...
| **무인 실행 루프(§4 PHASE 2 무인 모드) 가 변경 필요 발견** | ✅ **본 절차 필수** | 루프가 BLOCKER 로 정지 → Jira 이슈 자동 등록 → 본 절차 진입. 무인 루프는 베이스라인 산출물(SRS·DBML·Swagger·UI·TCL) 을 *직접 수정하지 않는다* |
